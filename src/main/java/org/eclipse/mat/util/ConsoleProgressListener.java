...
public class ConsoleProgressListener implements IProgressListener {
  ...
  public void beginTask(String name, int totalWork) {
    out.write(
        MessageUtil.format(Messages.ConsoleProgressListener_Label_Task) + " " + MessageUtil.format(
            name) + "\n"); //$NON-NLS-1$ //$NON-NLS-2$
    out.write("["); //$NON-NLS-1$
  ...
  public void subTask(String name) {
    out.write("\n"
        + MessageUtil.format(Messages.ConsoleProgressListener_Label_Subtask)
        + " "
        + MessageUtil.format(name)
